...
const { processTimetableData, groupAndSortTimetable, insertFreePeriods } = require('./timetableProcessor');
...
async function getTimetable(startDate) {
      ...
      const raw = await fetchTimetableData(sessionId, personType, personId, startDate);
      ...
      const processed = await processTimetableData(raw.elementPeriods[personId], filtered);
