...
async function processTimetableData(dataElementPeriods, filteredData) {
    ...
    return dataElementPeriods.map((period) => ({
        ...
        teacherName:
            teachersMap[period.elements.find((e) => e.type === 2)?.id]?.name ||
            "",
        room:
            roomsMap[period.elements.find((e) => e.type === 4)?.id]?.name || "",
        subject_short:
            lessonsMap[period.elements.find((e) => e.type === 3)?.id]?.name ||
            "",
...
function groupAndSortTimetable(timetable) {
    const grouped = timetable.reduce((acc, entry) => {
        if (
            entry.subject_short.includes("EBC") ||
            entry.subject_long.includes("EBC")
        ) {
            return acc;
        }
    ...
    for (const [date, entries] of Object.entries(groupedTimetable)) {
        ...
        entries.forEach((entry) => {
            ...
            if (
                ...
                entryStartTime - previousEndTime >= MAX_BREAK_DURATION
            ) {
